...
df_joined$Qm3_s <- df_joined$Q*0.0283
df_joined$Simm3_s <- df_joined$Sim*0.0283

dp <- ggplot(data=df_joined, mapping=aes( x = Date))+
  geom_line(mapping = aes(y = Qm3_s, color= "Measured"), show.legend = FALSE)+
  geom_line(mapping = aes(y = Simm3_s, color= "NHM Simulated"), show.legend = FALSE)+
...
#converting from ft3/s to m3/s
df_joinedB$Qm3_s <- df_joinedB$Q*0.0283
df_joinedB$Simm3_s <- df_joinedB$Sim*0.0283
...
#converting from ft3/s to m3/s
df_joinedC$Qm3_s <- df_joinedC$Q*0.0283
df_joinedC$Simm3_s <- df_joinedC$Sim*0.0283
...
dpC <- ggplot(data=df_joinedC, mapping=aes( x = Date))+
  ...
  labs(colour = NULL)+theme(axis.title.x=element_blank(),
                            axis.text.x=element_blank(),
...
#converting from ft3/s to m3/s
df_joinedD$Qm3_s <- df_joinedD$Q*0.0283
df_joinedD$Simm3_s <- df_joinedD$Sim*0.0283


dpD <- ggplot(data=df_joinedD, mapping=aes( x = Date))+
  geom_line(mapping = aes(y = Qm3_s, color= "Measured"))+
  geom_line(mapping = aes(y = Simm3_s, color= "NHM Simulated"))+
  scale_x_date(expand = c(0,0)) +
  #scale_x_continuous(breaks = seq(1980,2016,4))+
  scale_color_manual(labels=c('Measured', 'NHM Simulated'), 
                     values = c('Measured'='black', 'NHM Simulated'=col.cat.blu))+
...
#FDC Training and testing 

FDCtrainTest <- read_csv("C:/Users/p739b253/OneDrive - University of Kansas/FEWtures/Bias correction_NHM/FDCtraintest.csv")

#changing the date format to standard unambigious standard
FDCtrainTest$Date <-mdy(FDCtrainTest$Date)

#converting from ft3/s to m3/s
FDCtrainTest$QAm3_s <- FDCtrainTest$QA*0.0283
FDCtrainTest$FDCSimCorrectedAm3_s <- FDCtrainTest$FDCSimCorrectedA*0.0283

#converting from ft3/s to m3/s
FDCtrainTest$QBm3_s <- FDCtrainTest$QB*0.0283
FDCtrainTest$FDCSimCorrectedBm3_s <- FDCtrainTest$FDCSimCorrectedB*0.0283

#converting from ft3/s to m3/s
FDCtrainTest$QCm3_s <- FDCtrainTest$QC*0.0283
FDCtrainTest$FDCSimCorrectedCm3_s <- FDCtrainTest$FDCSimCorrectedC*0.0283

#converting from ft3/s to m3/s
FDCtrainTest$QDm3_s <- FDCtrainTest$QD*0.0283
FDCtrainTest$FDCSimCorrectedDm3_s <- FDCtrainTest$FDCSimCorrectedD*0.0283


A <- ggplot(data=FDCtrainTest, mapping=aes( x = Date))+
  geom_line(mapping = aes(y = QAm3_s, color= "Measured"), show.legend = FALSE)+
  geom_line(mapping = aes(y = FDCSimCorrectedAm3_s, color= "FDC"), show.legend = FALSE)+
  scale_x_date(expand = c(0,0)) +
  #scale_x_continuous(breaks = seq(1980,2016,4))+
  scale_color_manual(labels=c('Measured', 'FDC'), 
                     values = c('Measured'='black', 'FDC'=col.cat.red))+
  labs(y= 'Q (m3/s)')+
  ggtitle('Gage A')+
  theme(axis.title.x=element_blank())+
  #axis.text.x=element_blank())+
  theme(plot.title = element_text(vjust = -15))+
  geom_vline(xintercept = as.Date("2000-12-31"), color="blue", size=1, linetype="dashed")       #inserting vertical line
A

B <- ggplot(data=FDCtrainTest, mapping=aes( x = Date))+
  geom_line(mapping = aes(y = QBm3_s, color= "Measured"), show.legend = FALSE)+
  geom_line(mapping = aes(y = FDCSimCorrectedBm3_s, color= "FDC"), show.legend = FALSE)+
  scale_x_date(expand = c(0,0)) +
  #scale_x_continuous(breaks = seq(1980,2016,4))+
  scale_color_manual(labels=c('Measured', 'FDC'), 
                     values = c('Measured'='black', 'FDC'=col.cat.red))+
...
C <- ggplot(data=FDCtrainTest, mapping=aes( x = Date))+
  geom_line(mapping = aes(y = QCm3_s, color= "Measured"), show.legend = FALSE)+
  geom_line(mapping = aes(y = FDCSimCorrectedCm3_s, color= "FDC"), show.legend = FALSE)+
  scale_x_date(expand = c(0,0)) +
  #scale_x_continuous(breaks = seq(1980,2016,4))+
  scale_color_manual(labels=c('Measured', 'FDC'), 
                     values = c('Measured'='black', 'FDC'=col.cat.red))+
...
D <- ggplot(data=FDCtrainTest, mapping=aes( x = Date))+
  geom_line(mapping = aes(y = QDm3_s, color= "Measured"))+
  geom_line(mapping = aes(y = FDCSimCorrectedDm3_s, color= "FDC"))+
  scale_x_date(expand = c(0,0)) +
  #scale_x_continuous(breaks = seq(1980,2016,4))+
  scale_color_manual(labels=c('Measured', 'FDC'), 
                     values = c('Measured'='black', 'FDC'=col.cat.red))+
